## DELIVERY ROUTE v2 (owner directive 2026-09-26, supersedes contract 2026-09-11 §5 route)

WORK = FORK ONLY. PRs open on sternnick/incha (head sternnick:<branch> -> base sternnick:master).
VERIFIED 2026-09-26: PR-create on fork = PERMITTED (422 probe, never 403); PR #11 (fix/castinfo-arity)
and #12 (fix/weekly-review-2026-09-25) opened live. Upstream oseias-pt writes: 403 unchanged -> only
STAGE comment text, do not block on it. Fork issues: DISABLED (HTTP 410) - findings live on PR bodies.
Branch push to fork: permitted EXCEPT branches creating/updating .github/workflows/* (no `workflow`
scope) -> known victim: feature/snapshot-tests (16b). Split workflow edits out or ask owner for scope.
Fork master stays a PURE MIRROR: never merge into it; merge happens on PRs only when owner approves.
The old 'fork-only PRs are INVISIBLE to developer -> do not open one' rule is REVOKED: opening the PR
on the fork IS the delivery mechanism per owner.
...
## Lane snapshot 2026-09-26 (this delivery branch)

| Finding / work | State | Branch / URL |
|---|---|---|
| snapshot tests (Priority 1 item 2) | pushed to fork + PR opened | feature/snapshot-tests-split — workflow hunk split OUT per contract v2; code identical to feature/snapshot-tests @cba7dea; gates all.sh 16/16 rc=0 (9 matched) + luacheck 0/92f; checks.yml step text staged PENDING_PUSH 16c |

(Full table = cron-state copy; this is the contract-§6 snapshot committed on the branch.)
...
| weekly review 2026-09-25 @ de1dea6 | **DELIVERED — fix/weekly-review-2026-09-25 @247fbb9 PUSHED to fork** | 3 lanes clean sweep (no wrapper timeout). NEW fix: unwrapped engine callbacks (Panel.lua:233/238 scene StateChange, Preview.lua:83 RegisterForUpdate — throw = engine silently kills them; Xalvakka.lua:106 pattern applied). Gates named-worktree: all.sh 15/15 rc=0, luacheck 0/91f, 0-behind de1dea6. Upstream PR STAGED 21a (pulls 403, 13th). Falgravn equivalent = comment patch for #224 (21b, apply --check clean). New issues staged: HM 100000001 sentinel live-wrong under real dsr/rg isHM gates (21c — sharpens '16/25 dead': math.huge half dormant), silent non-BEGIN discard ED:431 + dead legacy seam (21d), 112/438 unevidenced ids oc37/58 se44/82 (21e). manifest.lua negative test VERIFIED (last week UNVERIFIABLE). Report: .reviews/review-2026-09-25.md. |
...
| nightly delta 2026-09-24 21:2xZ @ de1dea6 | **PARTIAL UNBLOCK — 16a branch PUSHED** | Fork mirror restored owner-side (origin=upstream=de1dea6, rev-list 0/0) → carried-history gate went silent. fix/castinfo-arity pushed to fork rc=0 (tip 537ecb4 = code f078e6b + docs). snapshot-tests push still REJECTED (16th; updates checks.yml → workflow scope). Upstream pulls/issues 403 → PR 16a remains STAGED but is now one API call away. Gates in NAMED worktrees: 15/15 rc=0 + 0/91f; 16/16 rc=0 (9 matched) + 0/92f. Replays ka14/ss4. 4th quiet upstream night, 0 activity. Report: DAILY_REPORT.md on branch + cron-state. |
